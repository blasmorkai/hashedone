...
pub fn instantiate(deps: DepsMut, msg: InstantiateMsg) -> StdResult<Response> {
    let config = Config {
        ...
        collective_ratio: msg.collective_ration,
    };
...
pub mod exec {
    use cosmwasm_std::{to_binary, Addr, BankMsg, Order, SubMsg, SubMsgResult, Uint128, WasmMsg, SubMsgResponse};
    ...
    pub fn peer_instantiate_reply(deps: DepsMut, msg: SubMsgResult) -> StdResult<Response> {
        //Three steps before accessing the response: get submsg response, get its data, parse its data
